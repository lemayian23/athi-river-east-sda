  ...
  try {
    ...
    grid.innerHTML = ministries.map(m => `
      <div class="bg-gray-50 rounded-lg p-4 shadow hover:shadow-md transition-shadow">
        ...
        ${m.email ? `<p class="text-sm text-gray-600">✉️ ${m.email}</p>` : ''}
    ...
    console.error(err);
    grid.innerHTML = '<p class="text-red-500 col-span-full text-center">Error loading ministries. Please try again later.</p>';
  }
}

// Initialize
document.addEventListener('DOMContentLoaded', loadMinistries);